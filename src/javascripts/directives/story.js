...
function positionStoryPortraitAtf(heroCover) {
  const target = document.querySelector('.story-mediavine-sidebar-atf');
  const copy = heroCover.querySelector('.story-cover__copy');

  if (!target || !copy) {
    return;
  }

  if (!target._storyOriginalParent) {
    target._storyOriginalParent = target.parentNode;
    target._storyOriginalNextSibling = target.nextSibling;
  }

  target._storyPositionAtf = () => {
    if (window.matchMedia('(min-width: 900px)').matches) {
      copy.appendChild(target);
      return;
    }

    target._storyOriginalParent.insertBefore(target, target._storyOriginalNextSibling);
  };

  if (!target._storyPositionAtfBound) {
    window.addEventListener('resize', () => target._storyPositionAtf());
    target._storyPositionAtfBound = true;
  }
...
function applyImageCropFocus(el) {
  ...
  Array.prototype.forEach.call(el.querySelectorAll('figure figcaption'), caption => {
    const candidates = caption.querySelectorAll('.credit, span, p');
    let markerNode = null;
    let position = null;

    Array.prototype.some.call(candidates, candidate => {
      const match = (candidate.textContent || '').trim().match(/^\[awaylands-crop:([a-z-]+)\]$/i);

      if (!match || validPositions.indexOf(match[1].toLowerCase()) === -1) {
        return false;
      }
    ...
    if (figure) {
      figure.classList.add(`crop-${position}`);
    }
    markerNode.parentNode.removeChild(markerNode);
    if (!(caption.textContent || '').trim() && !caption.querySelector('img, a')) {
...
function firstStorySummaryParagraph(el) {
  const paragraphs = el.querySelectorAll('p');
...
function storyOverviewLink(storyPage, overviewHeadings, index) {
...
function buildAdvancedStoryOverview(el, storyPage, isStyleEdit) {
  ...
  const source = firstStorySummaryParagraph(el);
  ...
  for (let index = 0; index < 3; index += 1) {
    const item = storyOverviewLink(storyPage, overviewHeadings, index);
    if (item) {
...
function fillRelatedStoryCandidates(storyPage) {
  if (!storyPage) {
    return Promise.resolve();
  }

  if (storyPage._relatedStoryFallbackPromise) {
    return storyPage._relatedStoryFallbackPromise;
  }
  ...
  storyPage._relatedStoryFallbackPromise = fetch('/story-titles.json')
    .then(response => {
      if (!response.ok) {
        throw new Error('Related story index unavailable');
      }
...
export default {
  ...
  bind(el) {
    ...
    el.storyImagesLoaded = new ImagesLoaded(el, () => {
      alignPairedMediumFigures(el);
      el.storyLuminousGallery = new LuminousGallery(lightboxStoryImages(el), galleryOpts, opts);
    });
  },
  inserted(el) {
    prepareStory(el);
